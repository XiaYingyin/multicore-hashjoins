...
typedef struct arg_npo_sj_column {
  int32_t tid;
  hashtable_t ** ht;
  column_t *rel_fact;
  column_t *rel_dim;
 ...
 * @return number of matching tuples
 */
int64_t probe_hashtable_column(hashtable_t *ht[], int num, column_t *rel,
  ...
  for (i = begin; i < end; i++) {
    ...
    for (k = 0; k < num; k++) {
      ...
      intkey_t idx = HASH(rel->column[i], hashmask, skipbits);
      bucket_t * b = ht[k]->buckets + idx;
      int64_t matches_dim = 0;
      do {
        for (j = 0; j < b->count; j++) {
          if (rel->column[i] == b->tuples[j].key) {
            matches_dim++;
            /* TODO: we don't materialize the results. */
          }
        }
...
npo_sj_column_thread(void * param) {
  int i, rv;
  arg_npo_sj_column_t * args = (arg_npo_sj_column_t*) param;
  bucket_buffer_t **overflowbuf;
...
NPO_SJ_COLUMN(relation_t *relr, relation_t *rels, int nthreads) {
	...
	int i, j, rv;
	cpu_set_t set;
	arg_npo_sj_column_t args[nthreads];
	pthread_t tid[nthreads];
	pthread_attr_t attr;
	pthread_barrier_t barrier;
	column_t *fact = (column_t*)relr;
	column_t *dim = (column_t*)rels;

	int dims = DIM_NUM;
	num_fact = fact[0].num_tuples;
	num_fact_per_thd = num_fact / nthreads;

	ht = (hashtable_t**) malloc(dims * sizeof(hashtable_t*));
	for (i = 0; i < dims; i++) {
		num_dim = dim[i].num_tuples;
		uint32_t nbuckets = (dim[i].num_tuples / BUCKET_SIZE);
	...
	for (i = 0; i < nthreads; i++) {
		...
		/* assing part of the relS for next thread */
		for (j = 0; j < dims; j++) {
			num_dim = args[i].rel_dim[j].num_tuples;
	...
	/* now print the timing results: */
	print_timing(args[0].timer1, args[0].timer2, args[0].timer3,
			fact->num_tuples, result, &args[0].start, &args[0].end);
#endif
